		...
		public function login(string $nickName, string $userName, string $realName, ?string $password = null): bool
		{
			...
			while (true) {
				...
				if (preg_match('/^PING\s*:(.+?)$/', $this->buffer, $matches)) {
					$this->pong($matches[1]);
				} else if (preg_match('/^:(.*?)\s+(\d+).*?(:.+?)?$/', $this->buffer, $matches)) {
					...
					// We got 464, which means we need to send a password.
					} else if ($matches[2] == 464 || preg_match('/irc.znc.in.*?You need to send your password/i', $this->buffer)) {
						...
						if ($password !== null && !$this->writeSocket('PASS ' . $tempPass)) {
							return false;
						} else if (isset($matches[3]) && str_contains(strtolower($matches[3]), 'invalid password')) {
							echo "Invalid password or username for ({$this->remoteHost}).";
							return false;
						}
					}
				} else if (preg_match('/^ERROR\s*:/', $this->buffer)) {
